...
function sLabel() {
    const e = document.getElementById("sLabel");
    return e;
}
function demoButton() {
    const e = document.getElementById("demoButton");
    return e;
}
function wBits() {
...
const input_re = new RegExp("^\\s*(?:" + [re_inf,
    re_nan,
    re_fpc,
    re_bin,
    re_hex,
...
function explainBV(size) {
    const ew = wBits();
    const ep = pBits();
    const wIn = parseInt(ew.value, 10);
    const pIn = parseInt(ep.value, 10);
